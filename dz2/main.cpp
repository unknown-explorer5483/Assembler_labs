...
void findWord(char* string, char* output) 
{
    ...
    size_t closest = min({fSpace,fsqBracket,frBracket,fstrEnd,fcomma,fsemicolon});
...
void checksymbol(char* string,bool* correct,int* start,const char* symbol,const char* correctin,const char* incorrectin)
{
    if (*correct) {
        char* stringpos = string+*start;
        ...
        {
            cout << "Incorrect symbol \"" << stringpos[0] << "\", expected \""<<symbol<<"\"\n";
            *correct = false;
        }
    }
}

void findAndCheckNumber(char* string,bool* correct,int* start,const char* correctin,const char* incorrectin)
{
    if (*correct) {
        char* stringpos = string+*start;
        char output[20] = "";
...
void structtype(char* string,bool* correct,int* start)
{
    ...
    skipSpace(string,start);
    char* stringpos = string+*start;
    while (!(strncmp(stringpos,";",1) == 0) && !(strncmp(stringpos,"\0",1) == 0) && *correct)
    {
        checksymbol(string,correct,start,",","","");
        skipSpace(string,start);
        ident(string,correct,start);
...
void vars(char* string,bool* correct,int* start)
{
    skipSpace(string,start);
    char* stringpos = string+*start;
    while (!(strncmp(stringpos,"\0",1) == 0) && *correct && !(strncmp(stringpos,"}",1) == 0)) 
    {
        char output[20] = "";
        findWord(stringpos,output);
...
int main() {
    ...
    //char str[] =  "   unsigned char aer[8]; char record[80]; struct student { char name[22];char family[22];int old; } st1,ft; 
    char str[1000] = "";
    cout << "Enter string:" << "\n";
